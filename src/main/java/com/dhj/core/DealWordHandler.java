...
public class DealWordHandler {
    ...
    public void initWordFile() {
        ...
        File file = new File(System.currentTimeMillis() + PROJECT_NAME + "数据库设计文档.xml");
        ...
        try {
            ...
            for (Map<String, String> ele : tableList) {
                ...
                List<Map> data = tableInfoService.queryTableInfoByName(DATABASE_NAME, ele.get("tableName"));
                for (Map fields : data) {
                    String tableBody = "";
                    ...
                    tableBody = tableBody.replace("${serialNo}", ((BigInteger) fields.get("serialNo")).toString(10));
